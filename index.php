...
// TANGKAP PESAN ERROR DARI PROSES ADMIN
if (isset($_SESSION['pesan_admin'])) {
    $pesan = $_SESSION['pesan_admin'];
    unset($_SESSION['pesan_admin']); // Langsung hapus agar tidak muncul terus saat di-refresh
}
...
<head>
    ...
    <style>
        ...
        .title { font-size: 22px; font-weight: 700; color: #0f172a; margin: 0 0 8px 0; line-height: 1.3; }
        .subtitle { font-size: 14px; color: #64748b; line-height: 1.5; margin: 0 0 24px 0; }
        
        /* Role Toggle Button */
        .role-toggle {
            display: flex; background-color: #f1f5f9; border-radius: 999px;
            padding: 4px; margin-bottom: 32px; width: 100%; box-sizing: border-box;
        }
        .role-btn {
            flex: 1; padding: 10px 0; text-align: center; border-radius: 999px;
            cursor: pointer; font-size: 13px; font-weight: 700; color: #64748b;
            background: transparent; border: none; transition: all 0.3s ease;
        }
        .role-btn.active { background-color: #0056b3; color: white; box-shadow: 0 2px 8px rgba(29, 78, 216, 0.2); }

        /* Form Controls */
        .form-group { margin-bottom: 20px; text-align: left; }
        .form-group label { display: block; margin-bottom: 8px; font-size: 13px; font-weight: 700; color: #334155; }
        
        /* Input Wrapper for perfect icon alignment */
        .input-wrapper { position: relative; display: flex; align-items: center; width: 100%; }
        .input-wrapper .left-icon { position: absolute; left: 14px; color: #94a3b8; font-size: 18px; pointer-events: none; }
        
        .form-control {
            width: 100%; padding: 14px 16px; border: 1px solid #cbd5e1; border-radius: 12px;
            font-size: 14px; color: #0f172a; box-sizing: border-box; transition: 0.2s; background: #f8fafc;
        }
        .form-control:focus { outline: none; border-color: #3b82f6; background: #ffffff; box-shadow: 0 0 0 4px #eff6ff; }
        
        /* Jika input punya icon di kiri, geser teksnya */
        ...
        .checkbox-group input { margin: 0; width: 16px; height: 16px; cursor: pointer; accent-color: #1d4ed8; }
        .checkbox-group label { font-size: 13px; color: #64748b; cursor: pointer; margin: 0; font-weight: 500; }
        .forgot-password { font-size: 13px; font-weight: 600; color: #3b82f6; text-decoration: none; }
        .forgot-password:hover { text-decoration: underline; }

        /* Submit Button */
        .btn-submit {
            width: 100%; padding: 14px; background-color: #1d4ed8; color: white;
            border: none; border-radius: 12px; font-size: 14px; font-weight: 700;
            cursor: pointer; transition: 0.2s;
        }
        .btn-submit:hover { background-color: #1e40af; transform: translateY(-1px); box-shadow: 0 4px 12px rgba(29, 78, 216, 0.2); }

        /* Divider */
        .divider { display: flex; align-items: center; text-align: center; margin: 24px 0; color: #cbd5e1; }
        .divider::before, .divider::after { content: ''; flex: 1; border-bottom: 1px solid #e2e8f0; }
        .divider span { padding: 0 16px; font-size: 12px; font-weight: 700; color: #94a3b8; }
...
<body>
    ...
    <script>
        ...
        // Fungsi untuk toggle password pada form admin (Mata Rapi & Presisi)
        document.getElementById('togglePwd').addEventListener('click', function() {
            ...
            if (type === 'text') {
                ...
                this.style.color = '#1d4ed8'; // Biru saat terlihat
            } else {
                icon.setAttribute('icon', 'lucide:eye');
                this.style.color = '#94a3b8'; // Abu-abu saat tersembunyi
            }
        });
        
        // Cek URL, otomatis buka tab Admin jika dilempar dari proses_login.php
